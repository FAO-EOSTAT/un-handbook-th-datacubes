## UN Handbook - Chapter 6: Earth observation data cubes

This repository contains the reproducible analysis for Chapter 6 of the UN Handbook on Earth observation big data sources. The chapter demonstrates how to work with satellite image time series and convert Analysis Ready Data (ARD) image collections into regular Earth observation data cubes using the `sits` R package.
...
```shell
git clone https://github.com/FAO-EOSTAT/un-handbook-th-datacubes
```

After cloning, navigate into the project directory using `cd un-handbook-th-datacubes`. All subsequent commands should be executed from within this directory.
...
```shell
docker pull faoeostat/handbook:chp6
```
...
```shell
docker build -t "faoeostat/handbook:chp6" --file docker/Dockerfile .
```
...
docker run \
    ...
    --volume ${PWD}:/home/rstudio/handbook faoeostat/handbook:chp6
```
...
Once you have accessed RStudio in your browser, you need to open the project. The project files are available in the `handbook` directory, which is located in the RStudio user home directory. Navigate to this directory using the Files pane in RStudio. Then double-click on the `th_data_cubes.Rproj` file to open the project. This will configure RStudio with the correct working directory and project settings.
